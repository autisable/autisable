...
|---|---|---|---|
| D1 | **Inline banner sizes** for affiliate placements | Primary: **300×250** (IAB standard, mobile-friendly). Optional secondary: 468×60 in sidebar/footer. Skip 728×90 — too wide for mobile. | ✅ Approved — used in L5 |
...
| D4 | **Image size reference doc** for members & editors | `IMAGE_SIZES.md` at repo root covers every upload (avatar, cover, featured, OG, inline, status update, site logo). Inline helper text added under PostEditor's Featured Image and OG Image fields. ImageUpload component already had recommended sizes; nothing to add there. | ✅ |
...
| M3 | Self-ID tags on profile (3 colored chips, mutually exclusive Neuro pairs) | ✅ | Shipped with L1. `self_id_tags TEXT[]` on user_profiles, multi-select on profile edit, colored chips render on public profile (parent_guardian / neurodiverse / professional). |
| M4 | 5-role system + migrate Author → Member | ✅ | Schema CHECK constraint widened to `member / contributor / moderator / editor / admin` with idempotent ALTER + UPDATE that migrates legacy `author` rows to `member`. New `app/lib/roles.ts` is the single source of truth: `Role` type, hierarchy ranks, `hasAtLeast()` and capability helpers (`can.publishBlogPosts`, `can.moderateComments`, etc.). New `PATCH /api/admin/members/[id]` route updates role/status via service role with a guardrail that prevents demoting the last remaining admin (lock-out protection). `/admin/members` UI now has role + status dropdowns per row, optimistic updates with rollback, per-row error display, and a collapsible role reference panel. |
...
| L5 | Affiliate banner framework + 3 partners (LegalShield, APM, VizyPlan) | ✅ | New `affiliates` table with admin RLS + idempotent seed for the three launch partners. Server-side `pickAffiliate()` rotates per request and supports per-category targeting. Client `<AffiliateBanner>` renders an uploaded 300×250 or 468×60 banner image when present, falls back to a styled text card built from name + tagline + CTA so the framework ships before Joel's banner artwork. Sidebar slot wired into every blog post (between body and author bio). FTC "Sponsored" label + `rel="sponsored"` for SEO compliance. New `/admin/affiliates` page with inline editing of all fields, active/paused toggle, placement controls, and category filtering. Admin nav updated. |
